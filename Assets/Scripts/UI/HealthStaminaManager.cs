...
public class HealthStaminaManager : MonoBehaviour
{
    ...
    public Image[] staminaBlocks; // 스테미나 블록 이미지 배열
    ...
    public float staminaRegenDelay = 2f; // 스테미나 회복 대기 시간
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Space)) // 테스트용으로 스페이스바를 눌렀을 때 체력 감소
        {
            TakeDamage(1);
        }

        if (Input.GetKey(KeyCode.LeftShift)) // 달리기 시 스테미나 감소
        {
